...
def main() -> int:
    ...
    require('version: "0.19.3"' in read("SKILL.md"), "SKILL.md version must be 0.19.3", errors)
    repository_readme = ROOT.parents[1] / "README.md"
    if repository_readme.is_file():
        repository_readme_text = repository_readme.read_text()
        require(
            "Current version:** v0.19.3" in repository_readme_text,
            "README version must be 0.19.3",
            errors,
        )
        require(
